...
func TestRemoteConfigGetStringFallbacks(t *testing.T) {
	...
	if got := rc.GetString("forward.target_address", "union@example.com"); got != "union@example.com" {
		t.Fatalf("missing key fallback = %q, want %q", got, "union@example.com")
	}

	rc.configs["forward.target_address"] = ""
	if got := rc.GetString("forward.target_address", "union@example.com"); got != "union@example.com" {
		t.Fatalf("empty value fallback = %q, want %q", got, "union@example.com")
	}

	rc.configs["forward.target_address"] = "ops@example.com"
	if got := rc.GetString("forward.target_address", "union@example.com"); got != "ops@example.com" {
