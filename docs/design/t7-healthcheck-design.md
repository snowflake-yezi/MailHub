...
flowchart TB
    subgraph mgmt["mgmt-system :8080"]
        heartbeat["HeartbeatHandler 被动<br/>POST /internal/servers/heartbeat<br/>刷新 last_heartbeat + current_load"]
        scheduler["HealthScheduler 主动<br/>ticker 30s<br/>GET /internal/health<br/>timeout 5s"]
        state["MailServer 状态机<br/>status / last_heartbeat<br/>last_probe_at / probe_fail_count"]
        allocator["分配器<br/>只选 status=healthy<br/>且 postfix_status=synced"]
    end

    subgraph node["mail-node :8081"]
        startHeartbeat["startHeartbeat 每 30s<br/>POST /api/v1/internal/servers/heartbeat<br/>body: server_id / load"]
        health["GET /internal/health<br/>供 mgmt 主动探测"]
    end

    startHeartbeat -->|"X-Internal-Token"| heartbeat
    scheduler -->|"X-Internal-Token"| health
    heartbeat --> state
    scheduler --> state
    state --> allocator
```
...
```mermaid
flowchart TB
    probe["GET http://&lt;api_host&gt;/internal/health<br/>X-Internal-Token, timeout 5s"]
    ok{"成功？<br/>HTTP 200 且 code==0"}

    success["ProbeFailCount = 0<br/>LastProbeAt = now"]
    recover["status ∈ {down, degraded}<br/>→ status = healthy"]
    draining["status == draining<br/>→ 保持 draining<br/>运维意图，探测成功不动"]

    fail["ProbeFailCount += 1<br/>LastProbeAt = now"]
    down{"ProbeFailCount >= 5？"}
    degraded{"ProbeFailCount >= 3？"}
    setDown["status = down<br/>含 draining：真不可达即摘除"]
    ...
    success --> recover
    success --> draining
    ok -->|"否：网络错 / 非 200 / code != 0 / 超时"| fail
    fail --> down
    down -->|"是"| setDown
    down -->|"否"| degraded
    degraded -->|"是"| setDegraded
    degraded -->|"否"| keep
```
