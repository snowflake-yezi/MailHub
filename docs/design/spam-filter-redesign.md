...
> 日期：2026-07-20
>
> 状态：暂停（2026-07-24）；历史实施记录保留，S11/S12 暂不继续
>
> 目标版本：P0-P4 分阶段交付，P5 统计分类器与 Rspamd 仅保留为后续候选

> 当前优先级通知：节点注册发现与出站控制通道已提升为唯一 P0 架构主线，详见[实施计划](node-registration-control-channel-implementation-plan.md)。广告过滤保留当前已部署代码、表结构、审计数据和 `dual_shadow/false` 状态，不发布 `dual_filter`，不启用自动隔离，不继续真实样本补齐、阈值选择或 S11/S12 验收。只有节点通信迁移为保持现有过滤 bundle、状态上报、decision outbox 和隔离区链路兼容所必需的修改仍在范围内。
...
> S11 工具进展（2026-07-22，本地 working tree）：`filter-replay` 增加严格 manifest 批量模式，可从全 shadow graph 生成 label×action 矩阵、脱敏发件域分层、时间/域 split 泄漏检查、阈值邻近样本和 would-quarantine 清单。报告不输出 EML 路径、正文、Message-ID 或真实发件域。合成三类样本、uncertain 排除、shadow score、泄漏检测和脱敏回归测试通过；真实三类样本、双人复核、shadow 观测、业务签字和 canary 仍未完成，因此 S11 保持 in_progress。

> 工具发布（2026-07-22）：`filter-replay` 已从 `fc09a0d` 构建并安装到生产工具路径。生产仍保持 `filter.engine_mode=legacy` 与 `filter.auto_quarantine_enabled=false`；S11/S12 状态不因工具安装而提前完成。

> 生产 baseline（2026-07-22）：在主节点只读盘点 16 个 Maildir，实际纳入 46 封 2026-06-22 至 2026-07-21 的开发验证邮件。初标全部为 `other`，没有 `ad` 或 `transactional` 样本；按入站时间分为 `16/15/15` 后时间隔离通过，但仅有两个发件域哈希且均跨 split，域隔离失败。`ad-seed-v1` revision 1 的 signed bundle checksum 为 `f0bbb21be5b2880dc324ee0a27692881b435f6dccb9930c30e28240a890f8775`；两次回放报告 SHA256 均为 `986b77f97038427c8b6a55b4521ab1361d3c108a6adba75a64dd99271cf257b1`。矩阵为 46 个 `other -> allow`，near-threshold 和 would-quarantine 均为 0。受限目录 `/secure/filter-replay-s11-20260722` 为 0700，manifest、bundle 和报告均为 0600。该结果只证明工具确定性和现有正常邮件零动作，不足以选择阈值或进入 `dual_filter`；仍需 `dual_shadow` 收集真实三类流量、双人复核和业务签字。

> Shadow 启用（2026-07-22）：提交 `88e2650` 新增受限 `filter-shadow-ops`，生产制品 11,235,894 字节、SHA256 `1507f979547c4a32dae38379b3ff57c8d34dd3b0686b57352e022b8a3b358306`，build info 为 `vcs.revision=88e2650585e5b0fc51399c93a9639ec50c7f878c / vcs.modified=false`。发布前完整备份位于 `/opt/mgmt-system/backups/filter-p4-s11-shadow-88e2650-20260722-035901`，数据库 dump 82,741 字节、SHA256 `d682ecc1db9c5cec908177fce5e4fe5d7bf51e7983a29ccf74d1c4718a6f5b2a`。通过正式状态机发布空 manual revision 1 和全 shadow ad revision 1 后，两节点四条策略状态全部收敛为 `1/1` 且 checksum 一致、error 为空；随后切换全局 `dual_shadow`。两节点配置 snapshot 分别为 `5/5`、`4/4`，`filter.engine_mode=dual_shadow`、`filter.auto_quarantine_enabled=false` 且来源均为 global。mgmt health/ready 与两节点直连 authenticated health 均为 200，服务 active，切换后 mgmt/node error 计数为 0，decision/quarantine 仍为 0。该状态只开始 shadow 数据采集，不批准 `dual_filter` 或自动隔离。
...
| 2026-07-22 | S11 | in_progress | working tree | `filter-replay --manifest` 合成三类/uncertain、全 shadow candidate score、label×action 矩阵、时间/域隔离、域哈希、阈值邻近、would-quarantine、敏感内容不出报告及 byte-for-byte 确定性测试通过；mail-node 全量 test/vet 与 replay race 通过 | 工具和操作文档完成；生产 baseline 见后续记录，真实三类样本、双人复核和业务签字仍缺失，未发布 active revision，未切换 `dual_shadow/dual_filter`，未启用自动隔离 |
...
| 2026-07-22 | S11 | in_progress | `fc09a0d` | 干净 Linux/amd64 `filter-replay` 制品 7,606,223 字节，SHA256 `f379247fb0690ec4e5c475ebf392a55592da3b1a1b6d5d57edaca5a85c57e9b7`，build info 为 `vcs.revision=fc09a0d0b22f9ebf933f88eeebb7f3b82cc1bfcf / vcs.modified=false`；已安装到生产工具路径且哈希一致，未替换或重启 mail-node | 安装时尚未执行生产回放；后续 baseline 见下一条记录。未发布 active revision，未切换 `dual_shadow/dual_filter`，未启用自动隔离，S11/S12 继续 in_progress |
| 2026-07-22 | S11 | in_progress | production baseline | 46 封开发验证邮件按入站时间分为 `16/15/15`；两次回放报告 SHA256 均为 `986b77f97038427c8b6a55b4521ab1361d3c108a6adba75a64dd99271cf257b1`。46 个样本均为 `other -> allow`，时间隔离通过，域隔离失败（2 个域哈希跨 split），near-threshold 与 would-quarantine 均为 0 | 现有历史邮件没有 `ad/transactional` 样本，不能选择阈值或批准 `dual_filter`。生产保持 `legacy/false` 且无 active revision；下一步发布全 shadow bundle 并进入 `dual_shadow` 收集真实三类流量 |
| 2026-07-22 | S11 | in_progress | `88e2650` | `filter-shadow-ops` 干净 Linux/amd64 制品 SHA256 `1507f979547c4a32dae38379b3ff57c8d34dd3b0686b57352e022b8a3b358306`；正式发布空 manual 与全 shadow ad revision 1，四条节点策略状态均为 `1/1`、checksum 一致且 error 为空。两节点配置 snapshot 为 `5/5`、`4/4`，均应用 `dual_shadow/false` global 配置；mgmt health/ready 与双节点 authenticated health 200，服务 active，切换后 error 计数 0，decision/quarantine 为 0 | 回滚点 `/opt/mgmt-system/backups/filter-p4-s11-shadow-88e2650-20260722-035901`，dump 82,741 字节、SHA256 `d682ecc1db9c5cec908177fce5e4fe5d7bf51e7983a29ccf74d1c4718a6f5b2a`。当前仅采集 shadow 数据；没有真实 `ad/transactional` 样本、双人复核或业务签字，禁止 `dual_filter` 与自动隔离 |
