...
涉及 schema 的步骤还必须在 MariaDB 10.5 生产结构副本执行迁移、重复启动和恢复演练；涉及 Maildir 的步骤必须使用临时 `new/cur/tmp/quarantine/outbox` 目录完成崩溃点测试。生产证据应保存在访问受控的运维系统中，不得写入或提交到本仓库；公开文档只保留验收标准和不含环境信息的结果摘要。

### 18.6 公开开发进度

这里只记录代码与自动化测试状态。生产发布时间、主机信息、制品摘要、PID、备份路径、运行数据和回滚记录统一保存在仓库外的受控运维系统中。
...
| 2026-07-22 | S11 | in_progress | working tree | `filter-replay --manifest` 合成三类/uncertain、全 shadow candidate score、label×action 矩阵、时间/域隔离、域哈希、阈值邻近、would-quarantine、敏感内容不出报告及 byte-for-byte 确定性测试通过；mail-node 全量 test/vet 与 replay race 通过 | 工具和操作文档完成；真实三类样本、双人复核和业务签字仍缺失，未发布 active revision，未切换 `dual_shadow/dual_filter`，未启用自动隔离 |
