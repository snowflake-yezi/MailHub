...
当前外部 API 使用 `api_tokens.scopes` 逗号字符串鉴权。Token 由配置文件写入，管理端不可见，也无法完成应用命名、权限勾选、凭证轮换、过期控制和调用审计。Token 还以明文保存，无法满足开放平台长期扩展需要。
...
启动时会将现有 `api_tokens` 转换为管理端可见的外部应用和哈希凭证；旧明文行仅为回滚保留。找不到新凭证时，升级兼容期内仍会回退查询旧 `api_tokens`。旧 Token 沿用原 scope 语义：
...
迁移后的应用可在管理端继续编辑权限和签发新凭证。后续版本在调用方完成凭证轮换后移除旧明文表和回退查询。
