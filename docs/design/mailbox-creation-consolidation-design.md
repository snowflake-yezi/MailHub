...
本次调整目标是把创建邮箱能力统一收口到“邮箱管理”页面，域名池只负责服务器域名绑定、Postfix/DKIM 同步与 DNS 管理。邮箱管理页底部固定展示创建区域，支持单个/批量创建，并提供可选服务器、可选域名的互相联动选择。
...
| 编号 | 需求 | 说明 |
|------|------|------|
| R1 | 创建入口合并 | 创建邮箱只在邮箱管理页展示，创建区域始终位于页面最后 |
...
| 决策 | 状态 | 说明 |
|------|------|------|
| 创建区域位置 | 已确认 | 邮箱管理页最后，避免打断账号集合/回收站/集成邮箱管理 |
...
- 移除页面顶部“创建邮箱”按钮。
- 移除 tabs 中的“创建邮箱”。
- 保留 tabs：账号集合、回收站、集成邮箱。
- 在页面最后固定渲染“创建邮箱”section。
- 创建结果不再因 `setView('normal')` 被隐藏。
...
| `mgmt-system/web/src/pages/MailboxesPage.jsx` | 创建区域移至页面最后，新增服务器/域名联动、结果展示、CSV 下载 |
