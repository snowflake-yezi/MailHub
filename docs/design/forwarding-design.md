...
```
              ...
              │   Nginx :443              │
              │                           │
              │  /admin     → mgmt :8080  │  ← IP白名单 / BasicAuth
              │  /api/v1/*  → mgmt :8080  │  ← Token 鉴权
              │  /static/*  → 静态文件     │
              └───────────┬───────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────┐
│                    📊 mgmt-system（控制面 + API 网关）            │
│                                                                 │
│  Web后台：邮箱CRUD / 服务器管理 / 订单映射 / 过滤规则              │
│  API网关：邮箱创建 / 邮件查询（出票中心 + LLM）                    │
│  MIME预处理：附件剥离 / text提取 / JSON组装                       │
│  控制层：负载分配 / 健康检查 / 规则下发 / 状态机                   │
│  数据层：账号表 / 映射表 / 服务器池 / 域名池 / 过滤规则             │
└──────┬───────────────────┬───────────────────┬──────────────────┘
       │ 内调               │ 内调               │ 内调
       ▼                    ▼                    ▼
┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│ 📫 邮箱服务器1 │    │ 📫 邮箱服务器2 │    │ 📫 邮箱服务器N │
│ mail-node    │    │ mail-node    │    │ mail-node    │
│              │    │              │    │              │
│ Postfix :25  │    │ Postfix :25  │    │ Postfix :25  │
│ Dovecot      │    │ Dovecot      │    │ Dovecot      │
│ OpenDKIM     │    │ OpenDKIM     │    │ OpenDKIM     │
│ 过滤引擎     │    │ 过滤引擎     │    │ 过滤引擎     │
│ 转发服务     │    │ 转发服务     │    │ 转发服务     │
│ 生命周期管理  │    │ 生命周期管理  │    │ 生命周期管理  │
└──────┬───────┘    └──────┬───────┘    └──────┬───────┘
       │ SMTP 转发          │                    │
       ▼                    ▼                    ▼
┌──────────────────────────────────────────────────┐
...
```
外部 SMTP → Postfix :25 → Maildir
                              │
                              │ mail-node forward.Service 直接扫本地 Maildir
                              ▼
                         SMTP → union 邮箱
```
...
```
mail_servers          server_domains         domains
┌──────────────┐     ┌─────────────────┐    ┌─────────────┐
│ 国际机1 (机1) │────▶│ server=1,domain=1│───▶│ example.com  │
│              │     │ server=1,domain=2│───▶│ ticket.xyz   │
└──────────────┘     └─────────────────┘    └─────────────┘
                     
┌──────────────┐     ┌─────────────────┐    
│ 国际机2 (机2) │────▶│ server=2,domain=2│───▶ ticket.xyz
│              │     │ server=2,domain=3│───▶ order.vip
└──────────────┘     └─────────────────┘    
```
...
```
mgmt 后台勾选域名 → mgmt 调 mail-node internal API
    → mail-node 改 /etc/postfix/virtual_mailbox_domains（或 main.cf）
    → postmap + postfix reload
    → 返回成功 → mgmt server_domains 落库
```
...
外部 SMTP
    │
    ▼
Postfix smtpd :25 → virtual(8) → Maildir /var/mail/vhosts/{domain}/{local}/new/{msgfile}
                                                                          │
                                                                          │  mail-node 轮询扫描
                                                                          ▼
                                              ┌───────────────────────────────────────┐
                                              ...
                                              │                                        │
                                              │  1. 扫描所有域名的 new/ 目录           │
                                              │  2. 检查 X-Forwarded-By 头（防循环）   │
                                              │  3. 解析邮件头（from/to/subject）      │
                                              │  4. 调用 filter.Engine.Filter()        │
                                              │  5. 根据结果：                         │
                                              │     ├── pass → SMTP 转发到 union      │
                                              │     ├── flag → SMTP 转发（加标签）     │
                                              │     └── block → 记日志，不转发         │
                                              │               mv new/ → cur/          │
                                              │              （原件保留，API 仍可查）    │
                                              │  6. SMTP 发送：io.Copy 流式写入       │
                                              │  7. 转发成功 → mv new/ → cur/          │
                                              │  8. 转发失败 → 留在 new/ 下次重试      │
                                              └───────────────────────────────────────┘
                                                                          │
                                                                          │ SMTP (io.Copy, 恒32KB内存)
                                                                          ▼
                                                              union@example.com
```
...
```
  ┌──────────┐   mgmt 调用 DELETE      ┌──────────┐   活跃转发结束      ┌──────────────┐   定时 GC (24h)    ┌──────────┐
  ...
  └──────────┘                         └──────────┘                   └──────────────┘                    └──────────┘
        ...
        │                                    │ 对账超时（15min）               │ 如需恢复
        │                                    ▼                                ▼
        │                              mgmt Watchdog                    os.Rename(.trash/ → vhosts/)
        │                              重新下发 DELETE                  （手工或 API）
```
...
mgmt-system                        mail-node
    │                                  │
    │  ① DELETE /internal/mailboxes/   │
    │     order-001@example.com         │
    │ ──────────────────────────────▶  │
    │                                  │  ② 摘除 Postfix virtual map（拒收新信）
    │                                  │     postmap + postfix reload
    │                                  │  ③ 等待活跃转发完成（ActiveJobs == 0）
    │                                  │     最长等待 5min，超时强制继续
    │                                  │
    │                                  │  ④ os.Rename(
    │                                  │       "/var/mail/vhosts/example.com/order-001",
    │                                  │       "/var/mail/vhosts/.trash/order-001-1719123456")
    │                                  │     )  ← 原子操作，不阻塞 Postfix
    │                                  │
    │  ⑤ 确认 → status=SOFT_DELETED     │
    │ ◀──────────────────────────────  │
    │                                  │
    │                                  │  ⑥ 定时 GC: 扫描 .trash/
    │                                  │     超过 24h 的目录 → os.RemoveAll
```
