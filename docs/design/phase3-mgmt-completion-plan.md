...
> 版本: v0.3 | 日期: 2026-06-24 | 状态: **Phase 3 规划已更新；T4/T5 已闭环，后续进入部署/T6/T7/T8**
> 依据: `REQUIREMENTS_ANALYSIS.md` §2.1 / §2.4 / §4 + 2026-06-24 代码审查（见 context.md「下次会话」）
...
| # | 功能（需求条目） | 现状 | 目标 | 优先级 |
|---|------------------|------|------|--------|
| C1 | 邮箱账号集合管理（§2.1.1/§2.1.6） | 邮箱管理页能力不足，缺少面向运营的账号/密码台账与域名筛选 | 展示已注册邮箱账号集合，含账密、域名、服务器、状态，并可按域名筛选 | 🔴 P0 |
| C2 | 邮件 MIME 预处理（§2.4/§2.2.4） | 裸透传 mail-node 原始响应 | 结构化 `{subject,body,attachments}` | 🔴 P0 |
| C3 | 服务器健康检查（§2.1.3） | 无定时任务，仅被动收心跳 | 主动探测 + 超时降级 + 自动摘除 | 🔴 P0 |
| C4 | 域名感知负载分配（§2.1.3） | 只按 current_load，不按域名 | server_domains 表 + 按域名筛 + 最闲分配 | 🔴 P0 |
| C5 | 邮箱生命周期状态机（§2.1.5） | 3 态，无 GC，DELETE 不下发 | 四态 + 定时 GC + 重新启用 | 🟡 P1 |
| C6 | 鉴权体系（§4.4） | `/admin` 与 `/api/v1/admin/*` 零鉴权 | 后台 session 鉴权 + scope + 内部接口加固 | 🟡 P1 |
| C7 | 域名管理（§2.1.6） | 仅 store + config seed | CRUD API + 页面 + MX 状态检测 | 🟡 P1 |
| C8 | 批量幂等 / 缺失页面 / 停用下发 等 | 多处残缺 | 见 §3.8 | 🟢 P2 |
| E1 | 订单-邮箱 N:M 映射扩展（§2.1.2） | 代码仍偏 1:1，且会改变邮件查询语义 | 暂缓；后续按扩展方案评审 | 🟢 扩展 |
| O1 | mgmt 部署国际机 | 在用户本地 Windows | 上国际机 + MySQL + Nginx | 🔴 前置 |
| O2 | inet_protocols / 临时机清理 / Let's Encrypt | — | 运维收尾 | 🟢 |
...
## 3. MVP 代码现实状态（2026-06-24 审读）

当前代码已经完成 MVP 骨架，但有多处临时语义与 PRD 不一致，后续实现必须先修正这些地基。

| 模块 | 代码现状 | 与 PRD 的偏差 |
|------|----------|---------------|
| 邮箱账号模型 | ✅ T1 已补 `password/sync_status/sync_error/synced_at`，新账号密码可持久化 | 历史账号密码为空；还需在 T2/T3 收敛创建链路与台账页面 |
| 邮箱列表 | `ListMailboxes(page,size,status,orderID)` 只支持状态和 order/search；页面只显示 `#DomainID`、`#ServerID` | 无法按绑定域名筛选，也没有展示域名名、服务器名 |
| 外部创建 API | `POST /api/v1/mailboxes` 走 `allocator.Allocate`，只写 mgmt DB 与 server load | 没有下发 mail-node 创建 Dovecot/Postfix 账号，和“创建即注册可用邮箱”不一致 |
| 后台批量创建 | 会调用 mail-node `/internal/mailboxes`，但远端失败仍继续写本地记录 | 可能产生 mgmt DB 与 mail-node 实际账号不一致 |
| 幂等语义 | 批量创建遇到已存在邮箱返回 `fail/email already exists` | PRD 要求幂等返回已有地址，不应作为失败 |
| 域名管理 | 有 `Domain` model + seed，无 CRUD API/页面；无 `server_domains` 表 | 域名筛选、服务器绑定域名、域名感知分配都缺 DB/API 支撑 |
| 鉴权 | `/admin` 无 session；`/api/v1/admin/*` 被 Token 中间件放行；内部接口无 shared-secret | 管理后台和内部接口不满足生产安全要求 |
| Scope | `RequireScope` 定义未接线，且类型断言不是 `model.ApiToken` | 真接入前需要修正，否则可能 panic |
| 邮件查询 | `email.go` 仍按 `order_id` 查邮箱并裸透传 mail-node 响应 | 与当前“按邮箱维度查询 + MIME 结构化供给”的 Phase 3 主线不一致 |
| 生命周期 | 只有本地 `active/disabled/recycled`，停用不下发 mail-node | 不满足四态/停收/软删/GC 流程 |
| 健康检查 | 只有被动 heartbeat 写入，未消费 `last_heartbeat` | 无主动探测、超时降级、down/draining 摘除 |

**直接结论**：当前最痛点不是 N:M，而是 mgmt 还不像一个可信的邮箱账号控制台。Phase 3 应优先把账号台账、创建下发一致性、域名筛选/域名管理打牢。
