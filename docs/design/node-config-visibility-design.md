...
> **状态：v3 实施中（NC-P0 至 NC-P4 已完成，2026-07-13）** ｜ 已完成可靠下发、热应用、重启可观测、配置状态机与作用域选择器 UX。
...
### NC-P4：作用域选择器 UX（已完成，2026-07-13）

- [x] [NC-13] 抽取 `ConfigField` / `ConfigDrawer` / `NodeConfigDrawer` 共享组件。
- [x] [NC-13] `ConfigPage` 顶部加作用域选择器，统一全局/节点交互；服务器池配置按钮直达对应节点作用域。
